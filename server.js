...
app.use(cors({
    origin: (origin, callback) => {
        ...
        if (allowAllOrigins || configuredAllowedOrigins.includes(origin) || allowedOriginRegex.test(origin)) {
...
app.get('/api', (req, res) => {
    res.json({
        status: databaseReady ? 'ok' : 'fallback',
        databaseReady,
        useFallbackData,
        databaseError: databaseError ? databaseError.message : null
    });
});
...
const initializeDatabase = async () => {
    try {
        const schemaStatements = loadSchemaStatements();

        if (schemaStatements.length > 0) {
            for (const statement of schemaStatements) {
                await query(statement);
            }
            console.log('✅ Database schema loaded from database/schema.sql');
        } else {
            await query(`
...
app.post('/api/pending-reports', async (req, res) => {
    ...
    try {
        const incomingReports = Array.isArray(reports) ? reports : (report ? [report] : []);
        if (!incomingReports.length) {
            return res.status(400).json({ error: 'No reports provided' });
        }

        const savedIds = [];
        for (const item of incomingReports) {
            const rows = await query(
                'INSERT INTO pending_reports (office, submitted_by, report_data, status, remarks, submitted_at) VALUES (?, ?, ?, ?, ?, ?) RETURNING id',
                [
                    item.office || '',
                    item.submittedBy || item.submitted_by || null,
                    JSON.stringify(item.data || item.report_data || {}),
                    item.status || 'pending',
                    item.remarks || item.remark || null,
                    item.submittedAt || item.submitted_at || new Date().toISOString()
                ]
            );
            savedIds.push(rows?.[0]?.id ?? null);
        }

        res.json({ success: true, message: 'Reports submitted', ids: savedIds.filter(Boolean) });
    } catch (err) {
        console.error('Error:', err.message);
